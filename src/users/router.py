...
from src.users.schemas import UserCreate, UserWithPermissions
from src.users.service import create_user_service, get_user_by_id_service

user_router = APIRouter(prefix="/users", tags=["Users"])


@user_router.post("/root", response_model=UserWithPermissions, status_code=201)
...
@user_router.post("/", response_model=UserWithPermissions, status_code=201)
...
@user_router.get("/{user_id}", response_model=UserWithPermissions, status_code=200)
